const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/hls.min-CnzOsMMy.js","assets/app--zcYgD1A.js"])))=>i.map(i=>d[i]);
import{a as e,c as t,d as n,f as r,i,r as a,v as o}from"./app--zcYgD1A.js";function s(e){switch(e){case 1:case 2:case 3:return 0;case 4:return 2;case 5:return 1;default:return 0}}function c(e){if(typeof e!=`string`)return[];let t=e.matchAll(/<d[^>]*?p="(?<p>[^"]+)"[^>]*>(?<text>.*?)<\/d>/gs);return Array.from(t).map(e=>{let t=e.groups.p.split(`,`);return t.length>=8?{text:e.groups.text.trim().replaceAll(`&quot;`,`"`).replaceAll(`&apos;`,`'`).replaceAll(`&lt;`,`<`).replaceAll(`&gt;`,`>`).replaceAll(`&amp;`,`&`),time:Number(t[0]),mode:s(Number(t[1])),fontSize:Number(t[2]),color:`#${Number(t[3]).toString(16)}`,timestamp:Number(t[4]),pool:Number(t[5]),userID:t[6],rowID:Number(t[7])}:null}).filter(Boolean)}function l({data:e}){let{xml:t,id:n}=e;if(!n||!t)return;let r=c(t);globalThis.postMessage({danmus:r,id:n})}function u(){let e=`
        ${s.toString()}
        ${c.toString()}
        onmessage = ${l.toString()}
    `,t=new Blob([e],{type:`application/javascript`});return new Worker(URL.createObjectURL(t))}function d(e){return new Promise(async t=>{let n=await(await fetch(e)).text();try{let e=u();e.onmessage=n=>{let{danmus:r,id:i}=n.data;i&&r&&(t(r),e.terminate())},e.postMessage({xml:n,id:Date.now()})}catch(e){console.error(`Error parsing Bilibili Danmu:`,e),t(c(n))}})}var f=`/*!
...
`,M=class{constructor(e,t){this.art=e,this.danmuku=t,this.utils=e.constructor.utils;let{setStyle:n}=this.utils,{$controlsCenter:r}=e.template;n(r,`display`,`flex`),this.template={$controlsCenter:r,$mount:r,$danmuku:null,$toggle:null,$config:null,$configPanel:null,$configModes:null,$style:null,$stylePanel:null,$styleModes:null,$colors:null,$opacitySlider:null,$opacityValue:null,$marginSlider:null,$marginValue:null,$fontSizeSlider:null,$fontSizeValue:null,$speedSlider:null,$speedValue:null,$input:null,$send:null},this.slider={opacity:null,margin:null,fontSize:null,speed:null},this.emitting=!1,this.isLock=!1,this.timer=null,this.createTemplate(),this.createSliders(),this.createEvents(),this.mount(this.option.mount),e.on(`resize`,()=>this.resize()),e.on(`fullscreen`,e=>this.onFullscreen(e)),e.on(`fullscreenWeb`,e=>this.onFullscreen(e)),e.proxy(this.template.$config,`mouseenter`,()=>{this.onMouseEnter({$control:this.template.$config,$panel:this.template.$configPanel})}),e.proxy(this.template.$style,`mouseenter`,()=>{this.onMouseEnter({$control:this.template.$style,$panel:this.template.$stylePanel})})}static get icons(){return{$on:k,$off:O,$config:x,$style:A,$mode_0_off:S,$mode_0_on:C,$mode_1_off:w,$mode_1_on:T,$mode_2_off:E,$mode_2_on:D,$check_on:b,$check_off:y}}get option(){return this.danmuku.option}get outside(){return this.template.$mount!==this.template.$controlsCenter}get TEMPLATE(){let{option:e}=this;return`
